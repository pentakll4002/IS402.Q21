...
function UserOrders() {
  ...
  // Get all active orders (pending, processing, shipping)
  const { data: orders, isLoading } = useQuery({
    queryKey: ["user-orders"],
    queryFn: async () => {
      try {
        // Get orders from Supabase - not filtering by user_id since the column doesn't exist
        const { data: ordersData, error: ordersError } = await supabase
          .from("orders")
          .select("*")
          .not("status", "in", '("completed","cancelled","refunded")') // Only active orders
